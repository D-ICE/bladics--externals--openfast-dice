...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
    ...
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmRVyt
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmRAyt
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmRoll
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmRVxt
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmRAxt
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmYaw
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmRVzt
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmRAzt
    Re_Xferred = Re_Xferred + 1
    ...
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmTVyi
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmTVxi
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmTVzi
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmTAyi
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmTAxi
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%PtfmTAzi
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%YawBrRDyp
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%YawBrRDxp
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%YawBrRDzp
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%YawBrRVyp
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%YawBrRVxp
    Re_Xferred = Re_Xferred + 1
    ReKiBuf(Re_Xferred) = InData%YawBrRVzp
    Re_Xferred = Re_Xferred + 1
...
IF (ALLOCATED(u_out%SuperController) .AND. ALLOCATED(u1%SuperController)) THEN
  ...
  END DO
END IF ! check if allocated
  b = -(u1%PtfmPitch - u2%PtfmPitch)
  u_out%PtfmPitch = u1%PtfmPitch + b * ScaleFactor
  b = -(u1%PtfmRVyt - u2%PtfmRVyt)
  u_out%PtfmRVyt = u1%PtfmRVyt + b * ScaleFactor
  b = -(u1%PtfmRAyt - u2%PtfmRAyt)
  u_out%PtfmRAyt = u1%PtfmRAyt + b * ScaleFactor
  b = -(u1%PtfmRoll - u2%PtfmRoll)
  u_out%PtfmRoll = u1%PtfmRoll + b * ScaleFactor
  b = -(u1%PtfmRVxt - u2%PtfmRVxt)
  u_out%PtfmRVxt = u1%PtfmRVxt + b * ScaleFactor
  b = -(u1%PtfmRAxt - u2%PtfmRAxt)
  u_out%PtfmRAxt = u1%PtfmRAxt + b * ScaleFactor
  b = -(u1%PtfmYaw - u2%PtfmYaw)
  u_out%PtfmYaw = u1%PtfmYaw + b * ScaleFactor
  b = -(u1%PtfmRVzt - u2%PtfmRVzt)
  u_out%PtfmRVzt = u1%PtfmRVzt + b * ScaleFactor
  b = -(u1%PtfmRAzt - u2%PtfmRAzt)
  u_out%PtfmRAzt = u1%PtfmRAzt + b * ScaleFactor
  b = -(u1%PtfmSway - u2%PtfmSway)
  u_out%PtfmSway = u1%PtfmSway + b * ScaleFactor
  b = -(u1%PtfmSurge - u2%PtfmSurge)
  u_out%PtfmSurge = u1%PtfmSurge + b * ScaleFactor
  b = -(u1%PtfmHeave - u2%PtfmHeave)
  u_out%PtfmHeave = u1%PtfmHeave + b * ScaleFactor
  b = -(u1%PtfmTVyi - u2%PtfmTVyi)
  u_out%PtfmTVyi = u1%PtfmTVyi + b * ScaleFactor
  b = -(u1%PtfmTVxi - u2%PtfmTVxi)
  u_out%PtfmTVxi = u1%PtfmTVxi + b * ScaleFactor
  b = -(u1%PtfmTVzi - u2%PtfmTVzi)
  u_out%PtfmTVzi = u1%PtfmTVzi + b * ScaleFactor
  b = -(u1%PtfmTAyi - u2%PtfmTAyi)
  u_out%PtfmTAyi = u1%PtfmTAyi + b * ScaleFactor
  b = -(u1%PtfmTAxi - u2%PtfmTAxi)
  u_out%PtfmTAxi = u1%PtfmTAxi + b * ScaleFactor
  b = -(u1%PtfmTAzi - u2%PtfmTAzi)
  u_out%PtfmTAzi = u1%PtfmTAzi + b * ScaleFactor
  b = -(u1%YawBrRDyp - u2%YawBrRDyp)
  u_out%YawBrRDyp = u1%YawBrRDyp + b * ScaleFactor
  b = -(u1%YawBrRDxp - u2%YawBrRDxp)
  u_out%YawBrRDxp = u1%YawBrRDxp + b * ScaleFactor
  b = -(u1%YawBrRDzp - u2%YawBrRDzp)
  u_out%YawBrRDzp = u1%YawBrRDzp + b * ScaleFactor
  b = -(u1%YawBrRVyp - u2%YawBrRVyp)
  u_out%YawBrRVyp = u1%YawBrRVyp + b * ScaleFactor
  b = -(u1%YawBrRVxp - u2%YawBrRVxp)
  u_out%YawBrRVxp = u1%YawBrRVxp + b * ScaleFactor
  b = -(u1%YawBrRVzp - u2%YawBrRVzp)
  u_out%YawBrRVzp = u1%YawBrRVzp + b * ScaleFactor
...
IF (ALLOCATED(u_out%SuperController) .AND. ALLOCATED(u1%SuperController)) THEN
  ...
  END DO
END IF ! check if allocated
  b = (t(3)**2*(u1%PtfmPitch - u2%PtfmPitch) + t(2)**2*(-u1%PtfmPitch + u3%PtfmPitch))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmPitch + t(3)*u2%PtfmPitch - t(2)*u3%PtfmPitch ) * scaleFactor
  u_out%PtfmPitch = u1%PtfmPitch + b  + c * t_out
  b = (t(3)**2*(u1%PtfmRVyt - u2%PtfmRVyt) + t(2)**2*(-u1%PtfmRVyt + u3%PtfmRVyt))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmRVyt + t(3)*u2%PtfmRVyt - t(2)*u3%PtfmRVyt ) * scaleFactor
  u_out%PtfmRVyt = u1%PtfmRVyt + b  + c * t_out
  b = (t(3)**2*(u1%PtfmRAyt - u2%PtfmRAyt) + t(2)**2*(-u1%PtfmRAyt + u3%PtfmRAyt))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmRAyt + t(3)*u2%PtfmRAyt - t(2)*u3%PtfmRAyt ) * scaleFactor
  u_out%PtfmRAyt = u1%PtfmRAyt + b  + c * t_out
  b = (t(3)**2*(u1%PtfmRoll - u2%PtfmRoll) + t(2)**2*(-u1%PtfmRoll + u3%PtfmRoll))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmRoll + t(3)*u2%PtfmRoll - t(2)*u3%PtfmRoll ) * scaleFactor
  u_out%PtfmRoll = u1%PtfmRoll + b  + c * t_out
  b = (t(3)**2*(u1%PtfmRVxt - u2%PtfmRVxt) + t(2)**2*(-u1%PtfmRVxt + u3%PtfmRVxt))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmRVxt + t(3)*u2%PtfmRVxt - t(2)*u3%PtfmRVxt ) * scaleFactor
  u_out%PtfmRVxt = u1%PtfmRVxt + b  + c * t_out
  b = (t(3)**2*(u1%PtfmRAxt - u2%PtfmRAxt) + t(2)**2*(-u1%PtfmRAxt + u3%PtfmRAxt))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmRAxt + t(3)*u2%PtfmRAxt - t(2)*u3%PtfmRAxt ) * scaleFactor
  u_out%PtfmRAxt = u1%PtfmRAxt + b  + c * t_out
  b = (t(3)**2*(u1%PtfmYaw - u2%PtfmYaw) + t(2)**2*(-u1%PtfmYaw + u3%PtfmYaw))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmYaw + t(3)*u2%PtfmYaw - t(2)*u3%PtfmYaw ) * scaleFactor
  u_out%PtfmYaw = u1%PtfmYaw + b  + c * t_out
  b = (t(3)**2*(u1%PtfmRVzt - u2%PtfmRVzt) + t(2)**2*(-u1%PtfmRVzt + u3%PtfmRVzt))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmRVzt + t(3)*u2%PtfmRVzt - t(2)*u3%PtfmRVzt ) * scaleFactor
  u_out%PtfmRVzt = u1%PtfmRVzt + b  + c * t_out
  b = (t(3)**2*(u1%PtfmRAzt - u2%PtfmRAzt) + t(2)**2*(-u1%PtfmRAzt + u3%PtfmRAzt))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmRAzt + t(3)*u2%PtfmRAzt - t(2)*u3%PtfmRAzt ) * scaleFactor
  u_out%PtfmRAzt = u1%PtfmRAzt + b  + c * t_out
  b = (t(3)**2*(u1%PtfmSway - u2%PtfmSway) + t(2)**2*(-u1%PtfmSway + u3%PtfmSway))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmSway + t(3)*u2%PtfmSway - t(2)*u3%PtfmSway ) * scaleFactor
  u_out%PtfmSway = u1%PtfmSway + b  + c * t_out
  b = (t(3)**2*(u1%PtfmSurge - u2%PtfmSurge) + t(2)**2*(-u1%PtfmSurge + u3%PtfmSurge))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmSurge + t(3)*u2%PtfmSurge - t(2)*u3%PtfmSurge ) * scaleFactor
  u_out%PtfmSurge = u1%PtfmSurge + b  + c * t_out
  b = (t(3)**2*(u1%PtfmHeave - u2%PtfmHeave) + t(2)**2*(-u1%PtfmHeave + u3%PtfmHeave))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmHeave + t(3)*u2%PtfmHeave - t(2)*u3%PtfmHeave ) * scaleFactor
  u_out%PtfmHeave = u1%PtfmHeave + b  + c * t_out
  b = (t(3)**2*(u1%PtfmTVyi - u2%PtfmTVyi) + t(2)**2*(-u1%PtfmTVyi + u3%PtfmTVyi))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmTVyi + t(3)*u2%PtfmTVyi - t(2)*u3%PtfmTVyi ) * scaleFactor
  u_out%PtfmTVyi = u1%PtfmTVyi + b  + c * t_out
  b = (t(3)**2*(u1%PtfmTVxi - u2%PtfmTVxi) + t(2)**2*(-u1%PtfmTVxi + u3%PtfmTVxi))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmTVxi + t(3)*u2%PtfmTVxi - t(2)*u3%PtfmTVxi ) * scaleFactor
  u_out%PtfmTVxi = u1%PtfmTVxi + b  + c * t_out
  b = (t(3)**2*(u1%PtfmTVzi - u2%PtfmTVzi) + t(2)**2*(-u1%PtfmTVzi + u3%PtfmTVzi))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmTVzi + t(3)*u2%PtfmTVzi - t(2)*u3%PtfmTVzi ) * scaleFactor
  u_out%PtfmTVzi = u1%PtfmTVzi + b  + c * t_out
  b = (t(3)**2*(u1%PtfmTAyi - u2%PtfmTAyi) + t(2)**2*(-u1%PtfmTAyi + u3%PtfmTAyi))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmTAyi + t(3)*u2%PtfmTAyi - t(2)*u3%PtfmTAyi ) * scaleFactor
  u_out%PtfmTAyi = u1%PtfmTAyi + b  + c * t_out
  b = (t(3)**2*(u1%PtfmTAxi - u2%PtfmTAxi) + t(2)**2*(-u1%PtfmTAxi + u3%PtfmTAxi))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmTAxi + t(3)*u2%PtfmTAxi - t(2)*u3%PtfmTAxi ) * scaleFactor
  u_out%PtfmTAxi = u1%PtfmTAxi + b  + c * t_out
  b = (t(3)**2*(u1%PtfmTAzi - u2%PtfmTAzi) + t(2)**2*(-u1%PtfmTAzi + u3%PtfmTAzi))* scaleFactor
  c = ( (t(2)-t(3))*u1%PtfmTAzi + t(3)*u2%PtfmTAzi - t(2)*u3%PtfmTAzi ) * scaleFactor
  u_out%PtfmTAzi = u1%PtfmTAzi + b  + c * t_out
  b = (t(3)**2*(u1%YawBrRDyp - u2%YawBrRDyp) + t(2)**2*(-u1%YawBrRDyp + u3%YawBrRDyp))* scaleFactor
  c = ( (t(2)-t(3))*u1%YawBrRDyp + t(3)*u2%YawBrRDyp - t(2)*u3%YawBrRDyp ) * scaleFactor
  u_out%YawBrRDyp = u1%YawBrRDyp + b  + c * t_out
  b = (t(3)**2*(u1%YawBrRDxp - u2%YawBrRDxp) + t(2)**2*(-u1%YawBrRDxp + u3%YawBrRDxp))* scaleFactor
  c = ( (t(2)-t(3))*u1%YawBrRDxp + t(3)*u2%YawBrRDxp - t(2)*u3%YawBrRDxp ) * scaleFactor
  u_out%YawBrRDxp = u1%YawBrRDxp + b  + c * t_out
  b = (t(3)**2*(u1%YawBrRDzp - u2%YawBrRDzp) + t(2)**2*(-u1%YawBrRDzp + u3%YawBrRDzp))* scaleFactor
  c = ( (t(2)-t(3))*u1%YawBrRDzp + t(3)*u2%YawBrRDzp - t(2)*u3%YawBrRDzp ) * scaleFactor
  u_out%YawBrRDzp = u1%YawBrRDzp + b  + c * t_out
  b = (t(3)**2*(u1%YawBrRVyp - u2%YawBrRVyp) + t(2)**2*(-u1%YawBrRVyp + u3%YawBrRVyp))* scaleFactor
  c = ( (t(2)-t(3))*u1%YawBrRVyp + t(3)*u2%YawBrRVyp - t(2)*u3%YawBrRVyp ) * scaleFactor
  u_out%YawBrRVyp = u1%YawBrRVyp + b  + c * t_out
  b = (t(3)**2*(u1%YawBrRVxp - u2%YawBrRVxp) + t(2)**2*(-u1%YawBrRVxp + u3%YawBrRVxp))* scaleFactor
  c = ( (t(2)-t(3))*u1%YawBrRVxp + t(3)*u2%YawBrRVxp - t(2)*u3%YawBrRVxp ) * scaleFactor
  u_out%YawBrRVxp = u1%YawBrRVxp + b  + c * t_out
  b = (t(3)**2*(u1%YawBrRVzp - u2%YawBrRVzp) + t(2)**2*(-u1%YawBrRVzp + u3%YawBrRVzp))* scaleFactor
  c = ( (t(2)-t(3))*u1%YawBrRVzp + t(3)*u2%YawBrRVzp - t(2)*u3%YawBrRVzp ) * scaleFactor
  u_out%YawBrRVzp = u1%YawBrRVzp + b  + c * t_out
